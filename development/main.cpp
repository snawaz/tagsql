...
void test_select(tagsql::development::data_context & dc)
{
	...
	//auto items = dc.select(at::name, at::age, bt::title).from(author);//.where(book.author_id == 30);
	auto items = dc.select(at::name, at::age).from(author).where(true);//.where(book.author_id == 30);
