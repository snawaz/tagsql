...
namespace tagsql { namespace meta 
{
	...
	public:
		...
		auto meta_tables() -> std::vector<meta_table>
		{
		    ...
		    auto rtables = exec("SELECT table_name FROM information_schema.tables\
			...
			auto rcolumns = exec("select column_name,data_type,is_nullable,column_default \
			...
			for(auto const & c : rcolumns)
			{
				...
				c[2].c_str()[0] == 'Y',  //is_nullable
				!c[3].is_null(),         //has_server_default
			    };
		    ...
		    {
			auto it = std::find_if(tables.begin(),
				tables.end(),
				[&](meta_table const & t) { return t.name == key[0].c_str(); });
			assert(it != tables.end());
			auto ic = std::find_if(it->columns.begin(),
				it->columns.end(),
				[&](meta_column const & c) { return c.name == key[1].c_str(); });
			...
			auto it = std::find_if(tables.begin(),
				tables.end(),
				[&](meta_table const & t) { return t.name == fkeyinfo[0].c_str(); });
			assert(it != tables.end());
			
			auto ic = std::find_if(it->columns.begin(),
				it->columns.end(),
				[&](meta_column const & c) { return c.name == fkeyinfo[1].c_str(); });
			assert(ic != it->columns.end());
